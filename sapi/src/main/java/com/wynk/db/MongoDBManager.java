...
import com.wynk.dto.AmazonS3Publisher;
...
public class MongoDBManager {
    ...
    private void init(MongoDBConfig config) {
        ...
        try {
            ...
            if(config == null){
             ...
             String mongoDbHosts = config.getMongoHosts();
             logger.info("mongoDbHosts string is "+ mongoDbHosts);
             String[] mongoDbHostsList = mongoDbHosts.split(",");
             for(String hostAndPort : mongoDbHostsList){
                 String[] hostAndPortPair = hostAndPort.split(":");
                 String host = hostAndPortPair[0];
                 int port = ObjectUtils.getNumber(hostAndPortPair.length == 1 ? 27017:hostAndPortPair[1], 27017).intValue();
                 if(host != null){
                     ServerAddress hostServer = new ServerAddress(host, port);
                     seeds.add(hostServer);
                 }
             }
    ...
    public String addObject(String collectionName, String objectId, String jsonData) {
        ...
        if(objectId == null) {
            ...
            if(isLoggingEnabled){
                mDatabaseLogger.info("addObject: Collection - "+ collectionName +", idStr - "+objectId+", jsonData - "+jsonData+" TimeTaken : "+ (
    ...
    public String addObjectAndGetId(String collectionName, String jsonData) throws Exception {
        long startTime = System.currentTimeMillis();
        DBCollection collection = getDB().getCollection(collectionName);
        DBObject dbObject = (DBObject) JSON.parse(jsonData);
        ...
        {
            if(insert.getLastError().getException() != null)
            {
                MongoException exception = insert.getLastError().getException();
                throw new Exception(exception);
            }
        }
        if(isLoggingEnabled) {
            mDatabaseLogger.info("addObject: Collection - " + collectionName + ", jsonData - " + jsonData + " TimeTaken : " + (
                    System.currentTimeMillis() - startTime));
        }
        return getObjectIdString((ObjectId) dbObject.get(MONGO_ID));
    }
...
//            ObjectId mongoId = getObjectId((String)dbObject.get(MONGO_ID));
            ...
            if(isLoggingEnabled){
                mDatabaseLogger.info("addObject: Collection - "+ collectionName +", idStr - "+ idStr +", jsonData - "+jsonData+" TimeTaken : "+ (
                        System.currentTimeMillis() - startTime));
            }
    ...
    public DBObject getObject(DBCollection collection, String idStr, boolean useObjectIdAsId) {
        ...
        if(isLoggingEnabled){
            mDatabaseLogger.info("getObject: Collection - "+ collection.toString() +", query - "+ query.toString() +", TimeTaken : "+ (
                    System.currentTimeMillis() - startTime));
        }
        return result;
    }
    ...
    public DBObject getObject(String collectionName, Map queryParams) {
        ...
        DBObject result = collection.findOne(query);
        if(isLoggingEnabled){
            mDatabaseLogger.info("getObject: Collection - "+ collectionName +", query - "+ query.toString() +", TimeTaken : "+ (
                    System.currentTimeMillis() - startTime));
        }
        return result;
    }
    ...
    public List<DBObject> getObjects(String collectionName, Map queryParams) {
        ...
        if(isLoggingEnabled){
            mDatabaseLogger.info("getObjects: Collection - "+ collectionName +", query - "+ query.toString() +", TimeTaken : "+ (
                    System.currentTimeMillis() - startTime));
        }
        return resultsList;
    }
    ...
    public DBObject getObject(String collectionName, Map queryParams, Map keys) {
        ...
        if(isLoggingEnabled){
            mDatabaseLogger.info("getObject: Collection - "+ collectionName +", query - "+ query.toString() +", keys - "+ dbkeys.toString() +", TimeTaken : "+ (
                    System.currentTimeMillis() - startTime));
        }
        return result;
    }
...
//        return getObjects(collectionName, startPos, n, queryParams, new HashMap<String, Object>());
    ...
    public boolean unsetField(String collectionName, Map<String, Object> queryParams, Map<String, Object> fieldValueMap) {
        long startTime = System.currentTimeMillis();
        DBCollection collection = getDB().getCollection(collectionName);
        BasicDBObject query = new BasicDBObject(queryParams);
        logger.debug("query = "+query.toString());
        BasicDBObjectBuilder builder = BasicDBObjectBuilder.start("$unset", new BasicDBObject(fieldValueMap));
        DBObject setField = builder.get();
        logger.debug("Query to be run : " + setField.toString());
        WriteResult writeResult = null;
        try {
            writeResult = collection.update(query, setField, false, false);
            if(isLoggingEnabled){
                mDatabaseLogger.info("setField: Collection - "+ collectionName +", query - "+ query.toString() +" setField - "+ setField.toString() +" , TimeTaken : "+ (
                        System.currentTimeMillis() - startTime));
            }
            return true;
            ...
            LogstashLoggerUtils.createFatalExceptionLogWithMessage(e, ExceptionTypeEnum.INFRA.MONGO.name(), "",
                    "MongoDBManager.setField", "Error setField " + collectionName);
...
//        return writeResult.getLastError().ok();
    ...
    public List<DBObject> getObjectsInOidRange(String collectionName, Map queryParams, Map fieldParams, ObjectId start,
            ObjectId end) {
        List<DBObject> objList = new ArrayList<DBObject>();
    ...
    public boolean deleteAllObjectFromCollection(String collectionName){
        DBCollection collection = getDB().getCollection(collectionName);
        List<DBObject> dbObjectList = getAllObjects(collectionName);
