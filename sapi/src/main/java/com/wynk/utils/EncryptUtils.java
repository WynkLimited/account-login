...
//        String result = Base64.decodeBase64String(data);
    ...
    public static String encrypt_256(String valueToEnc, String password) throws Exception {
    ...
    public static String encrypt_256_numeric(String valueToEnc, String password, int offset) throws Exception {
        Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider());
        Key key = generateKey(password);
        Cipher c = Cipher.getInstance(ALGORITHM_256, "BC");
        c.init(Cipher.ENCRYPT_MODE, key);
        byte[] encValue = c.doFinal(valueToEnc.getBytes("UTF8"));
        byte[] encryptedValues = Base64.encodeBase64(encValue);
        StringBuilder ss = new StringBuilder();
        Arrays.toString(encryptedValues);
        for (byte s : encryptedValues
        ) {
            ss.append(s);
        }
        return ss.toString().substring(0, offset);
    }

    public static String decrypt_256(String encryptedValue, String password) throws Exception {
    	Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider());
	    Key key = generateKey(password);
	    Cipher c = Cipher.getInstance(ALGORITHM_256, "BC");
	    c.init(Cipher.DECRYPT_MODE, key);
  ...
  // encryption and decryption key
    public static String getDeviceKey() {
        String key;
        /* 20% android build was live with enc/decryp key as uid. Hence handling here.
         todo : Need to remove this once build(345) is deprecated */
        if (MusicDeviceUtils.isAndroidDevice()) {
          key =
              MusicBuildUtils.isSupportedByBuildNumber(MusicDeviceUtils.getBuildNumber(), MusicBuildConstants.ANDROID_ENCRYPTION_BY_DID_SUPPORTED)
                  ? MusicDeviceUtils
                  .getDeviceId().concat(ENCRYPTIONKEY) : ChannelContext
                  .getUid();
      } else {
          key = MusicDeviceUtils
              .getDeviceId().concat(ENCRYPTIONKEY);
        }
...
//        } 
        ...
        String key2 = "MY$AIRTEL@2015#@";
        try {
        	String str = "404100620047141";
        	String key="kZlqMrt9LPyIt3wx22".substring(0, 16);
        	System.out.println("value "+str);
			//String encrypt = EncryptUtils.encrypt_256_numberic("404100620047141", key,8);
            String encrypt = EncryptUtils.encrypt("404100620047141", key);
