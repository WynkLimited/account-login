...
/**
 * This class is used to build xml document without using either SAX or DOM. single instance can be
 * used to create multiple documents.
 * 
 * @author Bhuvan
 */
public class XMLBuilder {
    ...
    /**
     * Changes the string used for indentation default intendation is "  ".
     * 
     * @param str
    ...
    /**
     * starts the xml document with default version and encoding default version = 1.0 default
     * encoding = UTF-8
     */
    ...
    public void startDocument(String version, String encoding) {
        reset();
        if(mode != END_DOCUMENT)
            throw new UnsupportedOperationException("Invalid state in startDocument: " + mode);
        depth = -1;
        buf.setLength(0);
    ...
    /**
     * starts the element of given name. Note that only starting tag of element will be added.
     * 
     * @param name
     */
    ...
    /**
     * adds the attribute to the current element. special character in value will be replaced by
     * their entities
     * 
     * @param name
     *            name of attribute
    ...
    public XMLBuilder addAttribute(String name, String value) {
        ...
        if(mode != START_ELEMENT)
            throw new UnsupportedOperationException("Invalid state in addAttribute: " + name + "=\"" + value + "\"");
     ...
     *            text to be added
     * @param cdata
     *            tell whether to enclose the text in CDATA section. if false, then special
     *            characters in given text are replaced by their entities.
     */
    public XMLBuilder addText(String text, boolean cdata) {
        if(text == null)
            text = "";
        if(mode == START_DOCUMENT)
            throw new UnsupportedOperationException("Invalid state in addText: " + mode);
        if(mode == START_ELEMENT)
            buf.append('>');
        if(mode == END_ELEMENT)
            printIndent();
    ...
    /**
     * adds and string containing xml to the current element this method does not verify whether the
     * given xml string. it wont replace any special characters in the given text.
     * 
     * @param text
    ...
    public XMLBuilder addXMLText(String text) {
        if(text == null)
            text = "";
        if(mode == START_DOCUMENT)
            throw new UnsupportedOperationException("Invalid state in addXMLText: " + mode);
        if(mode == START_ELEMENT)
            buf.append('>');
        buf.append(text);
        mode = TEXT_ELEMENT;
     ...
     * @param text
     *            text to be added if any. can be null
     * @param cdata
     *            tells whether the text should be enclosed in CDATA section. false will replace the
     *            special characters by their entities.
     */
    public XMLBuilder addElement(String name, String[][] atts, String text, boolean cdata) {
        startElement(name);
        if(atts != null) {
            for(int i = 0; i < atts.length; i++)
                addAttribute(atts[i][0], atts[i][1]);
        }
        if(text != null)
            addText(text, cdata);
        endElement(name);
    ...
    /**
     * resets this builder. if this instance is used to create multiple xml documents then make sure
     * that reset() is called before creating the document.
     */
    public void reset() {
        buf.setLength(0);
        stack.clear();
